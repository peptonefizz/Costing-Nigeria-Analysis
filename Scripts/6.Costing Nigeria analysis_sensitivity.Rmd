...
pacman::p_load(tidyverse,rio,janitor,here,sessioninfo,ggthemes)
...
```{r session info}
sessioninfo::session_info(pkgs = c("loaded", "attached")[1], to_file = here("Session info","Costing_Nigeria_analysis_sensitivity_session_info.txt"))
```
...
cost_input <- import(here("Data","cost_input_sensitivity_analysis.xlsx"), which = "cost") %>% 
  clean_names()


cost_range <- import(here("Data","cost_input_sensitivity_analysis.xlsx"), which = "cost_range") %>% 
  clean_names()
...
generate_cost_summary <- function(discount, ely_equipment, ely_training, format_tables = FALSE) {
  ...
  cost <- cost_input %>%
    ...
    mutate(unit_cost = ifelse(description %in% c("X-ray instrument", 
  ...
  for (i in 1:nrow(case_finding)) {
    ...
    if (quantity_col %in% c("quantity_Any_symptom", "quantity_Cough_≥_2_weeks")) {
      ...
      cost[[quantity_col]][cost$description %in% c("X-ray instrument", "AI licence")] <- 0
    }
...
```{r cost}
# Prepare all combinations of X-ray and AI licence from cost_range
xray_options <- cost_range %>%
  filter(description == "X-ray instrument")

ai_options <- cost_range %>%
  filter(description == "AI licence")

# Store all results
all_summaries <- list()

# Add base case from cost_input
base_summary <- generate_cost_summary(discount = 0.03, ely_equipment = 5, ely_training = 2, format_tables = TRUE)$summary_table %>%
  mutate(Source = "Base case")
all_summaries[[1]] <- base_summary

# Start sensitivity analysis loop
index <- 2
for (i in 1:nrow(xray_options)) {
  for (j in 1:nrow(ai_options)) {
    
    # Make a copy of cost_input to modify
    cost_input_scenario <- cost_input
    
    # Replace base costs with those from cost_range
    cost_input_scenario$unit_cost[cost_input_scenario$description == "X-ray instrument"] <- xray_options$unit_cost[i]
    cost_input_scenario$unit_cost[cost_input_scenario$description == "AI licence"] <- ai_options$unit_cost[j]
    
    # Assign temporary cost_input for the function to use (global trick)
    assign("cost_input", cost_input_scenario, envir = .GlobalEnv)
    
    # Run costing
    label <- paste("Xray:", xray_options$details[i], "| AI:", ai_options$details[j], ai_options$type[j])
    result <- generate_cost_summary(discount = 0.03, ely_equipment = 5, ely_training = 2, format_tables = TRUE)
    
    all_summaries[[index]] <- result$summary_table %>% mutate(Source = label)
    index <- index + 1
  }
}

# Combine and export all
summary_combined <- bind_rows(all_summaries)

export(list(summary_main = summary_combined), here("Output", "Sensitivity_analysis", "visualization_sensitivity.xlsx"))

```

```{r heatmaps}
# Extract AI and X-ray details from the Source column
summary_combined_viz <- summary_combined %>%
  mutate(
    # Extract values for sensitivity runs
    Xray = case_when(
      Source == "Base case" ~ "MinXray",
      TRUE ~ str_extract(Source, "(?<=Xray: ).+?(?= \\| AI:)")
    ),
    AI = case_when(
      Source == "Base case" ~ "Qure Annual",
      TRUE ~ str_extract(Source, "(?<=\\| AI: ).+")
    )
  ) 

# Unique list of algorithms
algorithms <- unique(summary_combined_viz$Algorithm)

# Create one heatmap per algorithm
cost_min <- min(summary_combined_viz$`Cost per case diagnosed`, na.rm = TRUE)
cost_max <- max(summary_combined_viz$`Cost per case diagnosed`, na.rm = TRUE)

# Directory to save the plots
output_dir <- here("Output", "Sensitivity_analysis", "Heatmaps")

# Create directory if it doesn't exist
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# Loop through each algorithm and save PNG
for (alg in algorithms) {
  plot_data <- summary_combined_viz %>%
    filter(Algorithm == alg)

   heat_plot <- ggplot(plot_data, aes(x = AI, y = Xray, fill = `Cost per case diagnosed`)) +
    geom_tile(color = "white") +
    geom_text(aes(label = round(`Cost per case diagnosed`, 0)), size = 3) +
    scale_fill_gradientn(
      colors = c("#ffffcc", "#a1dab4", "#41b6c4", "#225ea8"),
      limits = c(cost_min, cost_max),
      na.value = "grey90"
    ) +
    theme_minimal(base_size = 10) +
    theme(
      axis.text.x = element_text(angle = 0, vjust = 1.5, size = 7),
      axis.text.y = element_text(angle = 90, vjust = -1.5, hjust = 0.5, size = 7),
      axis.title.x = element_text(size = 11, face = "bold", margin = margin(t = 10)),
      panel.grid = element_blank()
    ) +
    labs(
      title = paste("Cost per TB Diagnosed –", alg),
      x = "AI Licence",
      y = "X-ray Instrument",
      fill = "Cost (USD)"
    )
  ...
  # Safe filename
  filename <- paste0("heatmap_", gsub("[^A-Za-z0-9]", "_", alg), ".png")

  # Save to file
  ggsave(filename = file.path(output_dir, filename),
         plot = heat_plot, width = 8, height = 6, dpi = 300)
}
```
